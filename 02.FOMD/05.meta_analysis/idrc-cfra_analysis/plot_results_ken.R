...
readr::write_csv(cereals.df1, paste0(path.metadata.effectsize, "/cereals_df_ken.csv"))
...
readr::write_csv(raw_diversification, paste0(path.metadata.effectsize, "/cereals_effects_ken.csv"))
...
raw <- read.csv(file.path(path.metadata.effectsize, "/cereals_effects_ken.csv"), stringsAsFactors = FALSE, check.names = TRUE)
...
ken_cereals <- ggplot(agg, aes(x = metric, y = practice, fill = category)) +
...
ggsave(paste0(path.metaanalysis,"/ken_cereals_heatmap.pdf"), plot = ken_cereals,
       width = 20, height = 15, dpi = 300, bg = "white")

#-----------------------------------------------------
#####-----ANALYSIS PER AGROECOLOGICAL ZONE -----------
#-----------------------------------------------------
ken.cereals <- read.csv(file.path(path.metadata.effectsize, "/cereals_df_ken.csv"), stringsAsFactors = FALSE, check.names = TRUE)
...
gaez_rasters <- list("Kenya" = ken.agr.zn)
...
sort(unique(ken.cereals.agr$out_indicator))

raw_ken.cereals.agr <- ken.cereals.agr %>%
  group_by(CT_crop_FAO_Food_Group_clean,
           diversification_spatial_temporal_theme,
           biomass_management_practice,
           nutrient_management_practice,
...
raw_ken.cereals.agr$row_id <- seq_len(nrow(raw_ken.cereals.agr))
changed_list <- lapply(seq_len(nrow(raw_ken.cereals.agr)), function(i) find_changed(raw_ken.cereals.agr[i, ]))
n_changed <- vapply(changed_list, length, integer(1))
...
agr.zn.single <- lapply(all_changed_rows, function(i) {
  ch <- changed_list[[i]]
  tibble(
    row_id = raw_ken.cereals.agr$row_id[i],
    agr_zone = raw_ken.cereals.agr$agr_zone[i],
    theme  = vapply(ch, `[`, character(1), 1),
    c_val  = vapply(ch, `[`, character(1), 2),
    t_val  = vapply(ch, `[`, character(1), 3)
  )
  ...
  bind_rows() %>%
  left_join(raw_ken.cereals.agr %>% select(row_id, impact, n, pos, neg), by = "row_id")
